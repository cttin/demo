...
Fm.prototype.bindEvent = function() {
    ...
    //   if(!_this.prevSong){
    //     _this.$prev.css('cursor', 'not-allowed');
    //     _this.$prev.attr('title', '前面没有歌曲啦~~');
    //   }else{
    //     _this.$prev.css('cursor', 'pointer');
    //     _this.$prev.attr('title', '上一首');
    //   }
    // })
...
Fm.prototype.getLrc = function(songId){
    ...
    $.get({
      ...
      _this.$node.find('.fm-lrc').html("本歌曲展示没有歌词");
    })
}
...
Fm.prototype.renderLrc = function (lrc) {
    ...
    $.each(lrcArr,function (key,value) {
        ...
        console.log(lrcLine[1],lrcLine[2], lrcLine[3]);
